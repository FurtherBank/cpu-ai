...
| 视频 URL +「对应合集」 | 用户给 BV 链接并说明要抓所在合集/播放列表 | **`b-claw -c <同一视频 URL> -a`**（`-c` 打开的是带侧边播放列表的页面即可，不必先解析 `collectiondetail` 链接） |
...
**禁止**为爬取、探路、解析合集 URL、确认登录态等目的调用 **Cursor MCP 浏览器**（`cursor-ide-browser`）或 **agent-browser** skill。常见误用：用户给视频链接并说「抓对应合集」时，Agent 打开 Cursor 浏览器点合集标签找 `collectiondetail` 链接——这是多余且错误的；应直接 `b-claw -c <视频 URL> -a`，由 `b-claw` 读取该页侧边播放列表。
...
1. 识别为「视频 URL + 对应合集」→ 选 **`b-claw -c "https://www.bilibili.com/video/BV1x4cveKEag" -a`**（**不要**开 Cursor 浏览器探合集链接）。
