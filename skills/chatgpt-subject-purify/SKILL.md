...
- 按用户输入用 `aichat sync` 同步指定时间窗内的 ChatGPT / Sider 对话，或对用户给出的 URL / id / 标题关键词做定向同步。
- 默认用 `aichat sync` 一次性完成远端扫描与内容拉取；仅输出有内容变动的对话。
- 需要查阅最近对话或按关键词定位本地对话时，必须用 `aichat view <可选关键词regex>`，再根据命令结果中的标题或路径访问实际对话文件。
...
**Subagent 返回格式（须包含）**：拉取阶段总结各字段（见下表）；可选 `behaviorLogPath`；`材料性影响摘要`（一行，供父日志影响清单）。

在 `cpu-cli-tool` 项目根执行。优先使用已安装的 `aichat` 命令；若不可用，再使用对应的 `node dist/commands/.../index.js` 入口。

### 批量同步（默认）

`aichat sync` 按**远端对话更新时间**筛选时间窗内条目，自动拉取完整内容并增量落盘。仅 stdout 输出有内容变动的对话，格式如：

```text
对话 xxx，更新时间2026-06-09 10:00:00，新增9轮，共12轮
```

默认同步最近 2 天（与整合阶段默认候选窗一致）：

```bash
aichat sync -o /Users/a1111/Desktop/ai-workspace/cpu-matrix/public/chats --days 2
```

整合阶段需要结构化交接时，加 `--json`；消费 `items` 中每条变更对话的 `title`、`updateTime`、`conversationDir`、`totalRounds` 等字段：
...
`public/chats/` 下原始对话目录很多，直接查看完整目录会非常低效，并且会引入大量相邻主题干扰。整合阶段需要查阅最近新对话、按标题关键词定位本地对话或补充候选来源时，必须先执行 `aichat view <可选关键词regex>`，按更新时间倒序查看结果，再根据命令结果中的文件名、标题或路径访问实际对话文件。
...
- `aichat sync` 会拉取时间窗内全部命中对话，但只有内容有变动的才输出；整合阶段候选范围以 `aichat sync --json` 的 `items`（或拉取阶段总结的候选清单）为准，再按主题边界筛选。
