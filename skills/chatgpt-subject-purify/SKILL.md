...
这个 Skill 与 `$info-purify` 共同构成一条流水线：本 Skill 承担“拉取、分类、筛选、归档路径和主题资产编排”，`info-purify` 承担“信息体系降熵的方法、校验和必要追溯”。主题主入口天然面向读者消费，承载核心上下文；台账和过程审计属于压缩附录或工作记录，只在会提升理解与维护质量时进入主入口。
...
| 2. 整合阶段 | 主控 Agent + `$info-purify` | 消费拉取阶段总结和源文件，完成主题筛选、结构归属、核心上下文与索引更新 | `核心上下文.md`、`索引.md`、必要概念文件 |
...
`核心上下文.md` 是主题的主入口：它承载的是可阅读、可推理、可复用、可继续讨论的正文，不承担维护台账、原始问答索引或强行给出终局判断的职责。
...
- 每块来源只记录大致角色，例如“核心材料”“补充案例”“边界反例”“相邻主题材料”“明确排除材料”；它与 `核心上下文.md` 的细粒度关系、单块判断、结论、推理链和详细摘要属于工作过程或核心正文，不属于索引。
- 纳入/排除/暂缓只表达边界理由，服务“哪些是、哪些不是”；索引不是内容分析表、事实承诺表、关系表、行动建议表或完整阅读指南。
- 细粒度来源回溯、逐轮问答梳理和模块级关系分析属于工作过程、自检记录或 `核心上下文.md` 的净化正文，不进入 `索引.md`。
...
- `核心上下文.md` 可以作为主题主入口继续讨论、判断、应用和维护。
- `索引.md` 只承担材料边界和粗粒度来源导航，不替代核心正文。
...
候选聊天会形成一份轻量筛选记录，用于执行过程判断和最终交付摘要；完整筛选记录默认属于工作过程，不写入 `索引.md`。记录至少包含：
...
`核心上下文.md` 的产物形态是完整可消费的主题上下文，不是大纲、原始材料清单或只剩结论的判断摘要。它的正文结构从主题内容本身长出来，用二级标题划分稳定内容范围；每个范围内部再按需要写定义、事实、观点、边界、关系、应用、当前可用判断和待确认。

`info-purify` 要求的结构诊断、目标结构、旧新映射、事实承诺、概念、关系、专项交接和升级候选，会在执行过程中形成并完成自检；进入 `核心上下文.md` 时，则按读者消费路径压缩和编排。主入口不是审计表集合。对长期维护、高风险或用户明确要求可追溯审计的主题，可以在 `核心上下文.md` 末尾保留必要表格，或另行生成工作记录；默认只有对理解和后续讨论有直接价值的部分进入主入口。
...
## 1. 任务基本性质
${info-purify 技能第一部分的 任务基本性质 内容}
## 2. 主题边界
直接说：如何判断特定内容是否属于这个主题，这个判断方法不要直接引用其它主题的上下文内容。
## 3..N. 主题内容模块
## 应用入口：后续讨论或具体问题时怎么用
## 当前可用判断与不可用判断
## 待讨论命题与研讨接口
...
- 核心正文不写来源清单、来源角色、上游依赖清单或“相关主题”清单；来源入口由 `索引.md` 承担，必要追溯只放读者理解强事实时必须知道的来源。
...
- 来源边界、相关主题入口和粗粒度来源入口属于索引或工作过程，不属于核心文件正文；候选筛选细节、原始问答逐轮路径和逐块关系分析默认不进入最终产物。
