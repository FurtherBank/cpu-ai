...
| 阶段 | 执行单元 | 目标 | 阶段输出 |
| --- | --- | --- | --- |
| 1. 拉取阶段 | 独立 Subagent，Subagent 自行读取 `workflow/pull-stage.md` 执行 | 获取 ChatGPT / Sider / 本地聊天候选材料，保持原始聊天资产不被改写 | 拉取阶段总结 |
| 2. 整合阶段 | 主控 Agent + `$info-purify` | 消费拉取阶段总结和源文件，完成主题筛选、结构归属、核心上下文、主题边界、索引与必要日志更新 | `核心上下文.md`、`主题边界.md`、`索引.md`、必要概念文件、必要时的净化日志记录 |
...
| 层级 | 默认文件 | 职责 |
| --- | --- | --- |
| 主题产物本体 | `核心上下文.md`、`主题边界.md`、必要概念文件 | 面向目标读者的最终正文、主题归入边界和概念扩展，可直接用于继续讨论、判断、应用和维护。 |
| 主题导航资产 | `索引.md` | 说明主题入口和明确依赖来源导航；只收录实际用于形成本主题最终内容的对话或直接依赖主题，不承载正文判断、主题边界定义、关联材料、相邻材料、排除材料、暂缓材料或过程审计。 |
| 净化日志记录 | `净化日志.md` / `工作记录.md`，或仅保留在交付说明中 | 记录拉取阶段总结、候选筛选、结构诊断、旧新映射、台账、事实概念关系核查、专项交接、升级候选和校验结果。 |

`核心上下文.md` 是主题产物本体和主入口：它承载的是可阅读、可推理、可复用、可继续讨论的正文，不承担维护台账、原始问答索引、候选筛选记录、主题归入边界定义或强行给出终局判断的职责。

`主题边界.md` 是主题下的固定文件：它承接原本 `核心上下文.md` 中“主题边界”章节的内容，定义本主题处理什么、不处理什么、如何判断特定内容是否属于本主题，以及当前使用方式。内容定义不变，只是从核心正文中独立出来。
...
| 文件 | 职责 |
| --- | --- |
| `核心上下文.md` | 净化后的主题正文：按内容结构聚合事实、观点、已确认判断、待讨论命题、模块边界、关系和使用方式。 |
| `主题边界.md` | 固定主题边界文件：承载原核心正文中的主题边界内容，说明本主题处理什么、不处理什么、当前使用方式和归入判断方法。 |
| `索引.md` | 主题导航资产：说明本主题入口和明确依赖来源入口；只收录实际用于形成本主题最终内容的对话或直接依赖主题，不承载主题边界定义、关联材料、相邻材料、排除材料或暂缓材料。 |
| 其他概念文件 | 当某个底层概念是全文评价轴时，单独承载定义、边界、非范围和关系约束。 |
| `净化日志.md` / `工作记录.md` | 可选过程记录：只在审计、长期维护、高风险或用户明确要求时落盘；默认不作为主题消费入口。 |

核心文件、`主题边界.md` 与 `索引.md` 天然分工：核心文件不重复主题边界定义和来源清单，也不逐轮列出原始问答事实；`主题边界.md` 只维护主题归入边界；原始问答回看路径由 `索引.md` 中明确依赖来源入口承担。涉及“问过什么/回应过什么”时，它们会被净化成对应内容模块下的必要事实、观点来源或待讨论命题；未实际参与形成最终内容的关联、相邻、排除、暂缓材料不进入索引。
...
| 层级 | 目标 | 默认产物 |
| --- | --- | --- |
| `chatgpt-subject-purify` | 管理主题资产边界、主入口、来源入口和长期维护方式 | 主题产物本体、固定主题边界文件、主题导航资产、必要时的净化日志记录 |
...
`索引.md` 不是主题正文、不是主题边界定义、不是判断摘要、不是候选材料审计表，也不是可替代 `核心上下文.md` 的使用入口。它只承担“明确依赖来源导航”的职责，避免读者误以为可以仅凭索引完成判断、复盘或二次创作。

索引的固定形态：

- 一个原始对话目录只有在其中内容被明确用于形成本主题最终内容时，才形成一行来源引用，链接到该对话目录或必要入口；即使目录下有 `001.md`、`002.md` 等多轮文件，也不逐轮展开完整清单，除非某一轮本身是唯一需要精确定位的例外来源。
- 其他主题只有在本主题最终内容直接依赖该主题的内容时，才可以作为直接依赖主题进入索引；该主题自己的依赖、相邻主题或背景来源不级联写入本主题索引。
- 每块来源只记录它如何参与形成本主题最终内容，例如“核心材料”“直接依赖主题”“关键事实来源”“关键观点来源”；它与 `核心上下文.md` 的细粒度关系、单块判断、结论、推理链和详细摘要属于净化日志记录或核心正文，不属于索引。
- 关联材料、相邻材料、排除材料、暂缓材料、背景材料、仅用于筛选的材料一律不进入索引；这些内容如需保留，只能进入净化日志记录、交付说明或不落盘。
...
| 已定型 | 规则、SOP、方法论已经明确，过程价值较低 | 可写成操作手册或稳定方法论，过程下沉到明确依赖来源或净化日志。 |
...
持续进行的同一个原始对话可能在后续轮次中改变讨论对象、消费路径或内容层级。遇到这种情况时，不能因为它仍在同一对话目录、同一标题或同一时间窗口里，就把后续内容整段归入原主题；必须按信息块重新判断主题归属，必要时把新轮次拆成独立主题、既有主题的新模块、相邻主题来源或排除材料。

归入既有主题的门槛高于“内容符合”。如果新的对话内容只是落在既有主题的宽泛命名范围内，不能直接归入；只有当它能和既有主题的明确脉络完全链接上，且实际补强、修正或延展某个已有稳定模块、事实承诺、概念边界或显式关系时，才归入既有主题。若只能说“这个内容也属于这个大方向”，但说不清它接到既有主题哪条脉络、改变哪个模块、使用哪个既有判断，就优先按新主题、暂缓或净化日志候选处理。
...
| 主题 | 是否围绕一个长期判断对象形成独立上下文系统？ | 新建或更新 subject，维护独立 `核心上下文.md`、`主题边界.md` 和 `索引.md`。 |
...
- 它能说明与既有主题脉络的精确接点，而不是只和主题名称或主题大方向相似。
...
- 它能明确链接到已有模块中的具体命题、概念、关系或使用入口；如果只能按宽泛主题名归类，不能归入已有模块。
...
- 同一对话持续进行后，后续轮次从原问题自然漂移到新的判断对象、应用对象或长期消费路径。

遇到这种情况时，材料不会为了整洁被整段压进一个主题。原始对话目录在索引中只形成一行来源入口；执行过程按信息块拆分归属，各自主题只吸收服务本主题的信息结构。`主题边界.md` 默认只说本主题自身，不借其他主题解释边界；其他主题没有被本主题正文实际使用时，就不进入核心正文。
...
| 用途 | 目录 |
| --- | --- |
| aichat CLI | `/Users/a1111/Desktop/ai-workspace/cpu-cli-tool` |
...
若项目 `AGENTS.md` 要求在一定规模任务前执行前置提交，例如 `cpu-matrix` 在主题净化、批量整理、索引与核心上下文重建等任务前要求提交已有工作区内容，则该规则优先于“不要提交用户改动”的通用保护。执行方式是：

- 在本 Skill 做拉取、写入 subject、生成日志等材料性动作前，先用目标项目根目录执行 `git status --short` 判断是否触发前置提交条件。
- 触发条件时，提交当前目标项目中已有未提交改动，即使这些改动不是当前 Agent 产生的；这是保存任务前基线，不是回滚、改写或审查用户改动。
- 前置提交只在对应项目仓库根目录执行，不在工作区根目录执行；提交信息使用“保存既有工作区改动”这类中性描述，或遵循项目既有提交风格。
- 前置提交后再开始本 Skill 的拉取和整合动作；本次主题产物完成后再单独提交本次产出，避免基线改动与本次产物混在同一个提交里。
- 若发现敏感文件、`.env`、密钥或项目规则明确禁止提交的文件出现在待提交列表中，停止并向用户说明阻断；否则不要因为“改动不是我产生的”而跳过项目规定的前置提交。
- 行为日志和交付说明中记录前置提交 hash、后续主题产物提交 hash，以及前置提交与本次产物提交的分工。
...
- `主题边界.md` 作为固定主题边界文件，承载本主题处理什么、不处理什么、当前使用方式和归入判断方法。
- `索引.md` 作为主题导航资产，只承担主题入口和明确依赖来源导航，不替代核心正文或主题边界文件；未实际参与形成最终内容的关联、相邻、排除、暂缓材料禁止进入索引。
...
主控 Agent 只负责确定主题名、目标目录、数据源范围和时间窗口，然后把拉取任务交给独立 Subagent。派发时只传递 `workflow/pull-stage.md` 路径和用户输入；主控 Agent 不打开该文件。Subagent 必须自行读取该流程文件，完成拉取、查阅、资产写入记录和“拉取阶段总结”返回。

拉取阶段不产出主题正文，不更新 `public/subjects/<主题>/`，也不替整合阶段做最终纳入判断。它只把材料拉齐、记录范围、暴露认证或覆盖缺口，并给出轻量候选清单。

**Audit Hook（拉取阶段）**：Subagent 返回后，父 Skill 更新行为日志「关键行为」「影响清单」（`public/chats/` 新增/确认）、「子 Skill 调用」（若返回 `behaviorLogPath`）或「关键行为」一行（纯 Task 摘要）。

## 阶段二：整合阶段

整合阶段由主控 Agent 执行，并同时调用 `$info-purify` 的信息降熵方法。它的输入是拉取阶段总结、原始聊天资产、既有 subject 文件和用户给出的主题边界；它的输出是主题资产更新。

这一阶段执行过程全程遵循`f-attention-budget-constraint`的约束要求。

**Audit Hook（整合阶段）**：每完成一次主题产物文件更新（核心上下文/主题边界/索引/概念文件/净化日志）后，更新行为日志「关键行为」「影响清单」「资源索引」；整合步骤 5 全部校验通过后，准备封口。
...
- `public/subjects/<主题>/主题边界.md`，如果存在。
...
- 默认候选范围：拉取阶段 `aichat sync --json` 输出中时间窗内已变更对话（`items`）；若未跑 `--json`，以拉取阶段总结的候选来源清单为准。
- 用户说“一天内 / 最近 N 天 / 某日期区间”时，拉取阶段应已用相同参数执行 `aichat sync`；整合阶段不再用本地 `filter` 或完整目录遍历替代远端同步。
- 用户给出具体对话 URL、id 或明确本地路径时，该对话可直接进入候选，不受 2 天默认窗口限制。
- 只有用户明确要求“全量”“历史所有聊天”或当前任务必须做历史回溯时，才扩大 `aichat sync` 时间范围，或通过 `aichat view <关键词regex>` 分批查阅候选；仍禁止直接扫描整个 `public/chats` 标题目录。

候选聊天会形成一份轻量筛选记录，用于执行过程判断和最终交付摘要；完整筛选记录默认属于净化日志记录，不写入 `索引.md`、`主题边界.md` 或 `核心上下文.md`。`索引.md` 只能从筛选记录中摘取已明确参与形成本主题最终内容的来源；排除、暂缓、相邻、关联、仅背景材料不进入索引。记录至少包含：
...
- 保留实际参与形成主题最终内容、能改变主题结构或直接支撑正文判断的材料。
- 每个候选对话先获得结构动作判断：新主题、新模块、归入已有模块、拆分归属、历史案例或升级候选；“纳入/排除”只是后续状态，不是完整判断。
- 允许同一个原始对话被拆成多个信息块，分别归入不同主题或模块；但原始对话目录只保留一个来源资产，不复制、不改写。
- 对持续推进的原始对话，按轮次和信息块检查讨论对象是否发生漂移；一旦后续内容换了消费路径，不沿用前半段的主题归属。
- 对新拉取的最近对话，若拉取阶段发现了目标之外的新候选材料，整合阶段必须单独登记：能独立成主题的单独整理，能精确链接到既有主题脉络的才更新既有主题，暂不确定的只进入筛选记录或净化日志，不因“内容符合”直接并入宽泛命名的旧主题。
- 归入既有主题时必须写出“脉络接点”：它接到哪个既有模块、补强哪条事实或关系、改变哪个边界或使用入口；没有脉络接点时，归入状态不得写成纳入既有主题。
...
- `aichat` 汇总的 ChatGPT 和 Sider 两个数据源进入同一份轻量筛选记录，用 `数据源` 字段区分；主题结构不按数据源拆成两套。
- 相邻主题材料只有在被本主题最终内容直接依赖时，才作为直接依赖主题进入索引；如果只是“关联、相邻、背景、可参考、未来接口”，不进入本主题索引。
- 纯案例只有在能提炼出模型、边界、流程或判断规则并实际进入主题最终内容时，才进入主题产物本体并可作为明确依赖来源进入索引；否则只在筛选记录、净化日志或交付说明中说明，不能作为补充/暂缓来源写入索引。
...
- `主题边界.md` 由本 Skill 固定维护，承接原本 `核心上下文.md` 中的主题边界章节，不等同于 info-purify 的日志记录。
- `索引.md` 由本 Skill 维护，是主题导航资产，不等同于 `info-purify` 的日志记录。

`核心上下文.md` 的产物形态是完整可消费的主题上下文，不是大纲、原始材料清单、审计表集合或只剩结论的判断摘要。它的正文结构从主题内容本身长出来，用二级标题划分稳定内容范围；每个范围内部再按需要写定义、事实、观点、模块边界、关系、应用、当前可用判断和待确认。主题整体归入边界不写在核心正文，固定进入 `主题边界.md`。
...
- `主题边界.md` 只写本主题自身：本主题处理什么、不处理什么、当前使用方式是什么；其他主题不用于解释本主题边界。
- 核心正文不写主题边界、来源清单、来源角色、上游依赖清单、候选筛选记录、旧新映射表或“相关主题”清单；明确依赖来源入口由 `索引.md` 承担，过程追溯由净化日志记录承担，正文只保留读者理解强事实时必须知道的来源锚点。
...
- 明确依赖来源入口属于索引；来源边界、相关主题入口、相邻主题入口、排除/暂缓材料和候选筛选细节属于净化日志记录或交付说明，不属于核心文件正文，也不属于索引。
...
3. 只把能服务模块主题的内容写入正文；原始顺序和逐轮问答不写入核心正文，索引只保留实际支撑这些正文内容的明确依赖来源入口。
...
- 顶部写明职责边界：`索引.md` 只说明主题入口和明确依赖来源导航；只收录实际用于形成本主题最终内容的对话或直接依赖主题。不承担主题正文、主题边界、详细摘要、逐轮回溯、行动建议、候选筛选、关联主题导航、相邻主题导航、排除材料记录或暂缓材料记录。
- 增加 `主题入口`：`核心上下文.md` 是完整上下文入口，`主题边界.md` 是固定主题边界入口，`索引.md` 只负责明确依赖来源导航。
- 底层概念文件存在时，列入主题入口或“底层概念”分组，只说明其入口身份。
- 若本主题最终内容实际引用其他主题作为直接依赖，在 `明确依赖来源` 中列出它的依赖角色；该主题自己的依赖不级联写入本主题索引。
- 若某个既有主题已完结且被实际用于形成本主题最终内容，可以作为“历史案例 / 验证来源”列入明确依赖来源；未参与形成最终内容则不列。
- 来源列表按“对话来源 / 直接依赖主题 / 必要概念文件”等角色分组即可；每个原始对话目录通常只写一行引用。
- 暂不纳入、排除、相邻、关联、背景、未来可能参考的材料一律不写入索引；如需说明，进入净化日志记录或交付说明。
...
> 本索引只说明本主题的入口和明确依赖来源：仅收录实际用于形成本主题最终内容的对话、直接依赖主题和必要概念文件。关联、相邻、排除、暂缓、背景、未来可能参考的材料禁止写入本索引；其他主题自己的依赖也不级联写入本索引。需要使用主题内容时，以 `核心上下文.md` 为入口；需要判断材料是否属于本主题时，以 `主题边界.md` 为入口。

## 主题入口
- [核心上下文](./核心上下文.md)：完整上下文入口。
- [主题边界](./主题边界.md)：主题归入边界入口。
...
### 直接依赖主题
- T01：[主题名](../<主题名>/核心上下文.md)：一句话说明本主题最终内容直接依赖了对方哪部分。
```
...
- 主题自身“哪些算入、哪些不算、如何判断归属”的稳定定义；这些应进入 `主题边界.md`。
- 关联、相邻、排除、暂缓、背景、未来可能参考的材料。
- 其他主题自己的依赖链；除非本主题最终内容直接依赖那些上游主题，否则不能级联写入。
...
1. Markdown 链接检查：确认 `索引.md`、`主题边界.md` 和 `核心上下文.md` 中的相对链接都能解析；若保留了历史 `核心判断.md`，也检查其指向新入口或不与新入口冲突。
2. 完整性检查：确认 `核心上下文.md` 不只是大纲、事实流水账、问答台账或结论清单，正文模块已经展开到“定义、模块边界、事实、观点、关系、应用、待确认、当前可用判断”；同时确认 `主题边界.md` 已承载主题整体归入边界。
3. 架构检查：确认底层概念的权威位置、非范围、关系约束已进入配套表格。
4. 污染检查：确认没有把排除材料、相邻主题、维护建议写成既定正文结论。
5. 分工检查：确认 `索引.md` 只承担主题入口和明确依赖来源入口，没有逐轮展开原始问答、没有写成详细摘要、主题边界或判断正文，也没有写入关联、相邻、排除、暂缓、背景、未来可能参考的材料；同时确认 `核心上下文.md` 没有重复索引来源清单，也没有内嵌主题边界章节。
...
10. 产物/日志边界检查：确认 `核心上下文.md` 是主题正文，`主题边界.md` 是固定主题边界文件，`索引.md` 是导航资产，候选筛选、旧新映射、完整事实承诺表、专项交接和主控校验没有混入三者；若需要保留，已进入 `净化日志.md` / `工作记录.md` 或交付说明。
11. 日志落盘检查：确认本次任务属于轻量、标准还是审计；若达到审计或用户要求可追溯，已经落盘净化日志记录；若未落盘，已说明理由和保留的关键风险。
12. 工作区检查：用 `git -C /Users/a1111/Desktop/ai-workspace/cpu-matrix status --short` 汇报工作区状态。若项目 `AGENTS.md` 触发前置提交规则，应先确认已有改动已作为前置基线提交；本次主题产物也应单独提交。只有在项目规则未要求前置提交、或用户明确要求不提交时，才保留并说明无关用户改动。
13. 行为日志检查：对照 [references/audit-quality-standards.md](references/audit-quality-standards.md) 与 `$behavior-log-audit` 的 [validate-log.md](../behavior-log-audit/workflow/validate-log.md)，确认本次 Skill 行为日志已封口或已声明审计缺口。
...
- `aichat sync` 拉取了哪些 ChatGPT / Sider 对话，写入或确认了哪些 `public/chats/...` 文件。
- `aichat view <可选关键词regex>` 查阅了哪些最近对话；哪些进入整合阶段候选，哪些因缺口暂缓。
- 主题目录中更新了哪些文件。
- `核心上下文.md` 是否已经可以作为主题主入口；若旧 `核心判断.md` 仍存在，说明它是保留、迁移还是暂未处理。
- `主题边界.md` 是否已经作为固定主题边界文件更新。
- `索引.md` 是否只作为主题导航资产更新，并且只包含实际参与形成最终内容的明确依赖来源。
- 是否生成或更新了 `净化日志.md` / `工作记录.md`；若没有，说明本次为何只在交付说明中保留关键日志。
- 是否有底层概念改变了架构位置。
- 校验是否通过，以及剩余风险。
- 本次 Skill 行为日志路径与终态（`.skill-logs/chatgpt-subject-purify/...`）。
