...
- P7. 所有「可行性存疑」的技术选型，已通过 PoC 验证或明确标注为已验证（参见 `workflow/poc.md`）
...
| `workflow/poc.md`           | 可行性存疑时的 PoC 验证流程                                 |
...
- 方案构思阶段先提出技术选型或改造路径，对可行性有把握时直接推进，不确定时触发 PoC 实验而非假设
...
- 技术选型可行性存疑（如某 SQL 语法在当前数据库版本不支持）：**不要凭经验猜测**，通过 PoC 验证后再写入设计
